...
class Personagem implements Comparable<Personagem> {
    ...
    public void read(String line) throws Exception {
        String[] info = TP4Q06.cutter(';', line);

        setId(info[0]);
        setName(info[1]);

        // alternate names
        String tmp = TP4Q06.shambles("[", "", info[2]);
        tmp = TP4Q06.shambles("]", "", tmp);
        String[] alternateNamesArray = TP4Q06.cutter(',', tmp);

        for (int i = 0; i < alternateNamesArray.length; i++) {
            String alternateSTR = alternateNamesArray[i].trim();
            alternateSTR = TP4Q06.shambles("'", "", alternateSTR);
        ...
        // alternate actors
        String tmp2 = TP4Q06.shambles("[", "", info[11]);
        tmp2 = TP4Q06.shambles("]", "", tmp2);
        String[] alternateActorsArray = TP4Q06.cutter(',', tmp2);
