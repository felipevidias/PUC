...
class Personagem implements Comparable<Personagem> {
    ...
    public void read(String line) throws Exception {
        String[] info = TP4Q01.cutter(';', line);

        setId(info[0]);
        setName(info[1]);

        // alternate names
        String tmp = TP4Q01.shambles("[", "", info[2]);
        tmp = TP4Q01.shambles("]", "", tmp);
        String[] alternateNamesArray = TP4Q01.cutter(',', tmp);

        for (int i = 0; i < alternateNamesArray.length; i++) {
            String alternateSTR = alternateNamesArray[i].trim();
            alternateSTR = TP4Q01.shambles("'", "", alternateSTR);
        ...
        // alternate actors
        String tmp2 = TP4Q01.shambles("[", "", info[11]);
        tmp2 = TP4Q01.shambles("]", "", tmp2);
        String[] alternateActorsArray = TP4Q01.cutter(',', tmp2);
    ...
    public void print(BinaryTree<Personagem> personagens, String id, int index) {
        ...
        if (personagem.getDateOfBirth() != null) {
            ...
            String[] dateCut = TP4Q01.cutter('-', date);
            date = dateCut[2];
            date += "-";
            date += dateCut[1];
            date += "-";
...
public class TP4Q01{
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (BufferedWriter BW = new BufferedWriter(new FileWriter("matricula_binarytree_java.txt"))) {
            BW.write("817294" + "\t" + names.countComp + "\t" + execTime + "ms");
        } catch (Exception e) {
            e.getStackTrace();
        }
